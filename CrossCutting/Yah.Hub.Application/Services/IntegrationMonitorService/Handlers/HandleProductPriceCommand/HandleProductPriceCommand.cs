...
namespace Yah.Hub.Application.Services.IntegrationMonitorService
{
    public partial class IntegrationMonitorService : IIntegrationMonitorService
    {
        ...
        /// <returns></returns>
        public async Task<ServiceMessage> HandleEntityState(CommandMessage<ProductPrice> serviceMessage)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (var oldState in currentState.Data)
                    {
                        ...
                        // get priceState
                        var oldPrice = oldState.ProductInfo.Skus.FirstOrDefault(x => x.Sku == serviceMessage.Data.AffectedSku.Id)?.PriceIntegrationInfo;
                        ...
                        // set new state
                        oldState.ProductInfo.Skus.FirstOrDefault(x => x.Sku == serviceMessage.Data.AffectedSku.Id).PriceIntegrationInfo = finalState;
        ...
        public PriceIntegrationInfo MergeEntityStates(CommandMessage<ProductPrice> source, PriceIntegrationInfo? oldState)
        {
            var newState = new PriceIntegrationInfo() { Retail = source.Data.AffectedSku.Price.Retail, DateTime = source.EventDateTime, SalePrice = source.Data.AffectedSku.Price.SalePrice, SalePriceFrom = source.Data.AffectedSku.Price.SalePriceFrom, SalePriceTo = source.Data.AffectedSku.Price.SalePriceTo };
